...
class ApiServices {
  ...
  Future<Result> getPopularMoviesByGenre(int genreId) async {
    var endPoint = 'discover/movie';
    final url = '$baseUrl$endPoint$key&with_genres=$genreId';

    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      return Result.fromJson(jsonDecode(response.body));
    }
    throw Exception('failed to load popular movies by genre');
  }
